...
GITHUB_FILE="prune-linux-arm64.tar.gz"
GITHUB_URL="https://github.com/BinaryPatrick/Prune/releases/download/${GITHUB_LATEST_VERSION}/${GITHUB_FILE}"

curl -s -L -o prune-linux-arm64.tar.gz $GITHUB_URL
tar xzf prune-linux-arm64.tar.gz ./prune
install -Dm 755 prune -t /usr/local/bin
rm prune prune-linux-arm64.tar.gz
